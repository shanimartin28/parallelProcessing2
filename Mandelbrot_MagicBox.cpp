...
/* Structure is used to store the border points of the new split image*/
struct borderPoints
{
    int point[4];
};

/* Queue to store border points of the split squares */
queue<borderPoints*> imagesQueue;
...
/*
 * Function to determine if the given image needs to be split into four new 
   images and to compute the fractal by calling the function computationMandel. 
 */
bool computeMagicBox(int yAxisMin, int xAxisMin, int yAxisMax, int xAxisMax)
{
    // Initially set the splitImage value to false
    bool splitImage = false;
    double Ymax= MaxIm;
    double Xmin= MinRe;
    int s_code = -1;
    
    /* Check the color codes returned by computationMandel
       If the color code is not the same, then set the 
       splitImage value to true*/
    for(int i = yAxisMin; i < yAxisMax; i++)
    {
        if(s_code<0)
        {
            s_code = computationMandel(i, xAxisMin, MaxIterations, ThresholdCheck, Ymax, Xmin, Im_factor, Re_factor);
        }
        if((s_code != computationMandel(i, xAxisMin, MaxIterations, ThresholdCheck, Ymax, Xmin, Im_factor, Re_factor))
            || (s_code != computationMandel(i, xAxisMax, MaxIterations, ThresholdCheck, Ymax, Xmin, Im_factor, Re_factor)))
        {
            splitImage = true;
            break;
        }
    ...
    for(int j = xAxisMin; j < xAxisMax; j++)
    {
        if(s_code<0)
        {
            s_code = computationMandel(yAxisMin, j, MaxIterations, ThresholdCheck, Ymax, Xmin, Im_factor, Re_factor);
        }
        if((s_code != computationMandel(yAxisMin, j, MaxIterations, ThresholdCheck, Ymax, Xmin, Im_factor, Re_factor))
          || (s_code != computationMandel(yAxisMax, j, MaxIterations, ThresholdCheck, Ymax, Xmin, Im_factor, Re_factor)))
        {
            splitImage = true;
            break;
        }
    }
    /* Else if the splitImage is not set, the color code
       is set to color_pixel*/
    if(s_code >= 0 && !splitImage)
    {
        int x,y;
        for(y = yAxisMin;  y< yAxisMax; y++)
            for(x = xAxisMin; x < xAxisMax; x++)
            {
                color_pixel[y][x] = s_code;
            }
    }
    /* Else if the splitImage value is set to true,
       Calculate the border points for the new split images */
    else
    {
        int s_ImageHeight = yAxisMax - yAxisMin;
        int s_ImageWidth  = xAxisMax - xAxisMin;

        if(s_ImageHeight > boxsize && s_ImageWidth > boxsize)
        {
            int s_ImageHeight_MidPt = yAxisMin + (int)((s_ImageHeight)/2);
            int s_ImageWidth_MidPt = xAxisMin + (int)((s_ImageWidth)/2);
            ...
            #pragma omp critical
            {
                ...
                borderPt->point[2] = s_ImageHeight_MidPt; 
                borderPt->point[3] = s_ImageWidth_MidPt;
                imagesQueue.push(borderPt);

                borderPt = new borderPoints;
                borderPt->point[0] = yAxisMin; 
                borderPt->point[1] = s_ImageWidth_MidPt; 
                borderPt->point[2] = s_ImageHeight_MidPt; 
                borderPt->point[3] = xAxisMax;
                imagesQueue.push(borderPt);

                borderPt = new borderPoints;
                borderPt->point[0] = s_ImageHeight_MidPt; 
                borderPt->point[1] = xAxisMin; 
...
*/
int computeMagicBoxAlgm()
{
    struct borderPoints *borderPt = NULL;
    /* Step1: Check border points for the given image.
       If the border points are not same split the image 
       into four new images, and store the corresponding 
       border points in the imagesQueue. */
    computeMagicBox(0, 0, ImageHeight, ImageWidth);
     
    #pragma omp parallel shared(imagesQueue)\
     private(borderPt)\
     num_threads(nthreads)
    {
        int queuesize = imagesQueue.size();
        /* Step2: While the imagesQueue contains border points data
            of the split images continue.*/
        while(queuesize > 0)
        {
            int yAxisMin = 0, xAxisMin = 0, yAxisMax = 0, xAxisMax = 0;
            #pragma omp critical
            {
                /* Step3: Obtain the border points data of the split image from 
                   top of the imagesQueue.*/
                borderPt = imagesQueue.front();
                if(borderPt)
                {
                    imagesQueue.pop();
                    ...
                    /* Step4: Delete the border points data of the current split image from 
                       the imagesQueue.*/
                    delete borderPt;
                    borderPt = NULL;
                }
            }
            /* Step5: Again make a call to the function computeMagicBox
                    to determine if the current image needs to be spilt .*/
            computeMagicBox(yAxisMin, xAxisMin, yAxisMax, xAxisMax);
            queuesize = imagesQueue.size();
        }
     }
    return 0;
}
...
main(int argc, char *argv[])
{
    ...
    printf("\nMandelbrot Magic Box Time Difference:%f\n",time_diff);
